...
| Consistency check UX Wave 1 (P0-07) | `Project.autoTerminologyScan` (default `true`); `TerminologyScanOnJobCompletedHandler`; `ConsistencySettingsPanel.tsx`; post-job toast; translations grid drift hints (`ProjectTranslationsPage.tsx`); Wave 2 LLM consistency reviewer deferred |
...
| Stale translation detection (P0-04 MVP) | `Translation.sourceTextSnapshot`; `stale-translation.utils.ts`, `StaleTranslationService`; hooks in `import-apply.service.ts`, `translation-key.handlers.ts`, `translation-job-runner.service.ts`; `stale-summary` / `stale-key-hints` API; `onlyStale` on job create; `ProjectTranslationsPage.tsx` stale filter + bulk retranslate; `ProjectOverviewTab.tsx` widget; `CreateJobModal.tsx` checkbox |
