...
**Slug:** `P0-02-excel-delta-import-shipped` · Reference spec — not active backlog.

> See [shipped-baseline](../shipped-baseline.md) and [demo/README](./README.md#shipped-tasks-reference). Client ideas #10 and #17 merged into this item.
...
| Layer | Change |
|-------|--------|
| **Parser** | `exceljs` parse + compose; Classic import preset (`Field ID \| Scope \| Key \| EN \| FR \| ES…`); empty-cell detection per language |
...
- Custom column-mapping wizard UI (API supports `custom` preset; UI ships Classic preset only)
- Before/after diff highlight in preview (sample rows only for MVP)

## Dependencies

- [P0-01](./P0-01-sport-domain-ai-context-shipped.md) recommended for translation quality
...
- [x] Import profile saved per project (Classic import preset ships out of box)
...
- Reuse `PlaceholderValidator` on every filled cell; failed rows stay empty in output + error column (Clients can re-import).

### Technical

- Parse with **`exceljs`** on backend (preserve formatting better than sheetjs); frontend `xlsx` only for small preview if needed.
- Classic import preset: versioned JSON fixture in `backend/test/fixtures/classic-export.xlsx` — e2e must assert column order + untouched Field IDs.
- Idempotency: same file hash + profile → skip re-translate unless `force=true`.
...
- **Import wizard** (modal or `/projects/:id/import`): Steps — Upload → Preset (Classic import) → Preview table (first 20 rows + “847 empty cells in FR”) → Languages → Start → Progress → Download.
...
| Wave 1 item | Move to Wave 2 after P0-03 file import unless Client sends Excel before Confluence export sample |
