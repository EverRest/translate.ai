    ...
    <div className="space-y-4">
      <div ref={dropdownRef} className="relative inline-block">
        ...
        >
          ...
          Add languages
          {pending.size > 0 && (
            <span className="rounded-full bg-sky-600 px-1.5 py-0.5 text-[10px] leading-none text-white">{pending.size}</span>
          )}
          <svg className={`h-3.5 w-3.5 text-slate-400 transition-transform ${open ? 'rotate-180' : ''}`} viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.5">
            <path d="M2 4l4 4 4-4" />
          </svg>
        </button>
        ...
        {open && (
          <div className="absolute left-0 top-full z-50 mt-1 flex w-72 flex-col rounded-xl border border-slate-700 bg-slate-800 shadow-2xl">
            ...
            <div className="max-h-64 overflow-y-auto py-1">
              {filtered.length === 0 && (
                <p className="px-3 py-4 text-center text-sm text-slate-500">No languages found</p>
              )}
              {filtered.map(l => (
                <label key={l.code} className="flex cursor-pointer items-center gap-3 px-3 py-2 hover:bg-slate-700">
                  <input
                    type="checkbox"
                  ...
                  <span className="text-xs uppercase text-slate-500">{l.code}</span>
                </label>
              ))}
            </div>
            ...
            <div className="border-t border-slate-700 p-2 flex gap-2">
              ...
              >
                {add.isPending ? 'Adding…' : `Add${pending.size > 0 ? ` ${pending.size}` : ''}`}
              </button>
              <button
                type="button"
                onClick={() => { setOpen(false); setSearch(''); setPending(new Set()); }}
      ...
      {!isLoading && (languages?.length ?? 0) === 0 && (
        <p className="text-sm text-slate-500">No target languages configured.</p>
      )}
        ...
        {languages?.map(language => {
          ...
          return (
            <li key={language.id}
              ...
              <span className="text-xs text-slate-500 uppercase">{language.code}</span>
              {!language.isDefault && (
                <button type="button" disabled={remove.isPending}
                  onClick={async () => { if (await confirm({ title: `Remove ${meta?.name ?? language.code}?`, description: 'This language and all its translations will be removed from the project.', danger: true, confirmLabel: 'Remove' })) remove.mutate(language.id); }}
