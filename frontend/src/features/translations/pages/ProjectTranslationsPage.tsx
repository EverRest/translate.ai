  ...
  useEffect(() => {
    ...
    es.onmessage = (e) => {
      ...
      if (data.type === 'translation') {
        onTranslation({ id: '', key: data.key, sourceText: data.sourceText, language: data.language, value: data.value, status: data.status, provider: null, version: 1 });
    ...
    es.onerror = () => { es.close(); onError('Could not reach the translation service.'); };
    return () => es.close();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [jobId]);
}

function BulkMenu({ onImport, onExport, onDeleteAll, isImporting, hasRows }: {
  onImport: () => void; onExport: () => void; onDeleteAll: () => void;
  isImporting: boolean; hasRows: boolean;
}) {
    ...
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-1 w-48 overflow-hidden rounded-lg border border-slate-700 bg-slate-800 shadow-xl">
          <button type="button" disabled={isImporting} onClick={() => { close(); onImport(); }}
          ...
          <button type="button" disabled={!hasRows} onClick={() => { close(); onExport(); }}
            className="flex w-full items-center gap-2 px-4 py-2.5 text-left text-sm text-slate-300 hover:bg-slate-700 disabled:opacity-50">
            ↓ Export Excel
          </button>
          <div className="my-1 border-t border-slate-700" />
          <button type="button" onClick={() => { close(); onDeleteAll(); }}
...
export function ProjectTranslationsPage() {
  ...
  const { data: translationsData, isLoading: translationsLoading } = useTranslations(projectId);
  ...
  const [filters, setFilters] = useState<GridFilters>({ search: '', status: '', sortField: 'key', sortDir: 'asc' });
  ...
  const { keys: allKeys, total: keysTotal, isLoading: keysLoading, hasMore: keysHasMore, loadMore: loadMoreKeys, refetch: refetchInfiniteKeys } =
  ...
  const handleSseTranslation = (t: Translation) => {
    setLiveTranslations((prev) => { const next = new Map(prev); next.set(`${t.key}:${t.language}`, t); return next; });
  };
  ...
  const rows = useMemo<TranslationRow[]>(() => {
    ...
    return allKeys.map((k) => ({
      keyId: k.id, key: k.key, sourceText: k.sourceText,
  ...
  const filteredRows = useMemo(() => {
    let result = rows;
    if (filters.status) {
      result = result.filter((r) => Object.values(r.translations).some((t) => t?.status === filters.status));
    }
    return [...result].sort((a, b) => {
      const field = filters.sortField;
      const cmp = a[field].toLowerCase() < b[field].toLowerCase() ? -1 : a[field].toLowerCase() > b[field].toLowerCase() ? 1 : 0;
  ...
  useEffect(() => {
    return () => {
      if (scrollEl.current) scrollEl.current.removeEventListener('scroll', onScrollHandler);
      roRef.current?.disconnect();
    };
  }, []);

  // Total virtual height uses server total (for correct scrollbar)
  const virtualTotal = filters.status ? filteredRows.length : Math.max(keysTotal, filteredRows.length);
  const totalHeight = virtualTotal * ROW_HEIGHT;
  const visibleStart = Math.max(0, Math.floor(scrollTop / ROW_HEIGHT) - OVERSCAN);
  const visibleEnd = Math.min(virtualTotal - 1, Math.ceil((scrollTop + containerHeight) / ROW_HEIGHT) + OVERSCAN);
  ...
  const handleComplete = async () => {
    ...
    if (activeToastRef.current) { toast.update(activeToastRef.current, 'Translation complete ✓', 'success'); activeToastRef.current = null; }
    setActiveJobId(null);
  };
  const handleError = (msg: string) => {
    setLiveTranslations(new Map()); setTranslatingKeys(new Set());
    if (activeToastRef.current) { toast.update(activeToastRef.current, msg, 'error'); activeToastRef.current = null; }
    setActiveJobId(null);
  };
  useJobSSE(activeJobId, projectId ?? '', handleSseTranslation, handleComplete, handleError);

  const startJob = async (keyNames: string[], keyIds: string[], label: string) => {
    if (!projectId) return;
    const targetLangs = languages.map((l) => l.code);
    if (targetLangs.length === 0) { toast.error('No target languages configured. Add languages in Settings.'); return; }
    const toastId = toast.loading(label);
    activeToastRef.current = toastId;
    setTranslatingKeys(new Set(keyIds));
    try {
      const job = await createJob({ projectId, languages: targetLangs, keys: keyNames });
      setActiveJobId(job.jobId);
    } catch (err) {
      toast.update(toastId, err instanceof Error ? err.message : 'Failed to start translation job.', 'error');
      activeToastRef.current = null; setTranslatingKeys(new Set());
    }
  };

  const handleTranslateAll = () => {
    if (filteredRows.length === 0) return;
    void startJob(filteredRows.map((r) => r.key), filteredRows.map((r) => r.keyId), `Translating ${filteredRows.length} key${filteredRows.length === 1 ? '' : 's'}…`);
  };
  const handleTranslateRow = (row: TranslationRow) => void startJob([row.key], [row.keyId], `Translating "${row.key}"…`);
  const handleDeleteRow = (row: TranslationRow) => {
    if (!window.confirm(`Delete key "${row.key}"? All translations will be removed.`)) return;
    deleteKey.mutate(row.keyId, { onError: (err) => toast.error(err instanceof Error ? err.message : 'Failed to delete key.') });
  };

  const toggleSort = (field: SortField) => {
    setFilters((f) => ({ ...f, sortField: field, sortDir: f.sortField === field && f.sortDir === 'asc' ? 'desc' : 'asc' }));
  };

  const handleDeleteAll = async () => {
    if (!projectId) return;
    if (!window.confirm('Delete ALL translations for this project? This cannot be undone.')) return;
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (keys.length === 0) { toast.update(toastId, 'No valid title rows found in file.', 'error'); return; }
      ...
      await Promise.all([refetchInfiniteKeys(), refetchKeys(), refetchTranslations()]);
    } catch (err) {
  ...
  const handleExport = () => {
    ...
    const wsData = [['Field Id', 'Type', 'Field', ...langCodes], ...rows.map((r) => [r.key, 'title', r.sourceText, ...langCodes.map((lang) => r.translations[lang]?.value ?? '')])];
  ...
  const isInitialLoading = (keysLoading && allKeys.length === 0) || translationsLoading;
  const isTranslating = translatingKeys.size > 0;

  const SortIcon = ({ field }: { field: SortField }) => {
    if (filters.sortField !== field) return <span className="ml-1 text-slate-600">↕</span>;
    return <span className="ml-1 text-sky-400">{filters.sortDir === 'asc' ? '↑' : '↓'}</span>;
  };
    ...
    <section className="flex h-[calc(100vh-200px)] flex-col gap-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-sm text-slate-400">
            {allKeys.length}{keysHasMore ? ` / ${keysTotal}` : ''} key{keysTotal === 1 ? '' : 's'}
            {debouncedSearch && ` matching "${debouncedSearch}"`}
            {keysLoading && allKeys.length > 0 && <span className="ml-2 text-slate-500">loading…</span>}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <input ref={importFileRef} type="file" accept=".xlsx,.xls" className="hidden" onChange={handleImport} />
          <button type="button" disabled={isTranslating || filteredRows.length === 0} onClick={handleTranslateAll}
          ...
          <BulkMenu onImport={() => importFileRef.current?.click()} onExport={handleExport}
            onDeleteAll={() => void handleDeleteAll()} isImporting={isImporting} hasRows={rows.length > 0} />
        </div>
      </div>
      ...
      {!isInitialLoading && languages.length === 0 && (
        <div className="rounded-lg border border-amber-800/50 bg-amber-950/20 px-4 py-3 text-sm text-amber-300">
          No target languages configured. Add them in <span className="font-medium">Settings → Languages</span>.
        </div>
      )}
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-slate-800">
        ...
        <div className="flex shrink-0 border-b border-slate-800 bg-slate-900">
          ...
          <div className="flex w-56 shrink-0 items-center gap-1 border-r border-slate-800 px-4 py-3">
            <button type="button" onClick={() => toggleSort('sourceText')}
              className="flex items-center text-xs font-medium uppercase tracking-wide text-slate-400 hover:text-white">
              {defaultLang?.code.toUpperCase() ?? 'Source'} <SortIcon field="sourceText" />
            </button>
          </div>
          {languages.map((lang) => (
            <div key={lang.id} className="flex w-48 shrink-0 items-center border-r border-slate-800 px-4 py-3 last:border-r-0">
              <span className="text-xs font-medium uppercase tracking-wide text-slate-400">{lang.code}</span>
            </div>
          ))}
        ...
        <div ref={containerRefCb} className="relative flex-1 overflow-x-auto overflow-y-auto bg-slate-900/40"
          ...
          {!isInitialLoading && filteredRows.length === 0 && (
            <div className="flex h-full items-center justify-center">
              <p className="text-slate-400">
                {filters.search ? 'No keys match your search.' : 'No translation keys yet.'}
              </p>
            </div>
          )}
          ...
          {!isInitialLoading && filteredRows.length > 0 && (
            <div style={{ height: totalHeight, position: 'relative' }}>
              <div style={{ position: 'absolute', top: 0, left: 0, right: 0, transform: `translateY(${offsetY}px)` }}>
                {visibleRows.map((row) => (
                  <GridRow key={row.keyId} row={row} languages={languages.map((l) => l.code)}
                    onTranslate={handleTranslateRow} onDelete={handleDeleteRow}
                    isDeleting={deleteKey.isPending && deleteKey.variables === row.keyId} />
                ))}
              </div>
            </div>
          )}
...
type GridRowProps = { row: TranslationRow; languages: string[]; onTranslate: (row: TranslationRow) => void; onDelete: (row: TranslationRow) => void; isDeleting: boolean };

function GridRow({ row, languages, onTranslate, onDelete, isDeleting }: GridRowProps) {
  return (
    <div className={['flex border-b border-slate-800 transition-colors hover:bg-slate-800/30', isDeleting && 'opacity-40', row.translating && 'bg-sky-950/10'].filter(Boolean).join(' ')}
      style={{ height: ROW_HEIGHT }}>
      <div className="flex w-48 shrink-0 items-center border-r border-slate-800 px-4">
        <span className="truncate font-mono text-xs text-slate-300" title={row.key}>{row.key}</span>
      </div>
      <div className="flex w-56 shrink-0 items-center border-r border-slate-800 px-4">
        <span className="truncate text-sm text-slate-200" title={row.sourceText}>{row.sourceText}</span>
      </div>
      {languages.map((lang) => {
        const t = row.translations[lang];
        return (
          <div key={lang} className="flex w-48 shrink-0 items-center border-r border-slate-800 px-2 last:border-r-0">
            {row.translating && !t ? (
            ...
            ) : t ? <TranslationCell translation={t} /> : <span className="text-xs text-slate-600">—</span>}
          </div>
        );
      })}
      <div className="flex w-28 shrink-0 items-center gap-1 px-2">
        <button type="button" disabled={row.translating || isDeleting} onClick={() => onTranslate(row)} title="Translate this key"
          className="rounded px-2 py-1 text-xs text-sky-400 hover:bg-sky-950/40 hover:text-sky-300 disabled:opacity-30">Translate</button>
        <button type="button" disabled={isDeleting} onClick={() => onDelete(row)} title="Delete key"
          className="rounded px-2 py-1 text-xs text-red-500 hover:bg-red-950/40 hover:text-red-400 disabled:opacity-30">Delete</button>
      </div>
    </div>
  );
}

function TranslationCell({ translation }: { translation: NonNullable<TranslationRow['translations'][string]> }) {
  ...
  const statusDot: Record<string, string> = { draft: 'bg-slate-500', review: 'bg-amber-500', approved: 'bg-emerald-500', published: 'bg-sky-500' };

  if (editing) {
    return <input ref={inputRef} value={localValue} onChange={(e) => setLocalValue(e.target.value)}
